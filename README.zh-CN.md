...
| 小红书 | `xiaohongshu/me`, `xiaohongshu/feed`, `xiaohongshu/search`, `xiaohongshu/note`, `xiaohongshu/comments`, `xiaohongshu/user_posts`, `xiaohongshu/search-page`, `xiaohongshu/note-detail`, `xiaohongshu/notes-chunk`, `xiaohongshu/comments-chunk` | 完整小红书支持，含批量导出工作流原子能力、分块笔记详情抓取与分块评论抓取 |
...
- `xiaohongshu/search-page`、`xiaohongshu/note-detail`、`xiaohongshu/notes-chunk`、`xiaohongshu/comments-chunk` 都是面向批量导出的工作流原语，字段名和分块契约尽量保持机器友好，方便 `bb-xhs-export` 这类下游工具直接消费。
